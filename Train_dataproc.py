...
df = spark.read.option("header",True).csv("/user/input/small_temp.csv")
df.show(5)
...
rf = RandomForestClassifier(numTrees=10,maxBins=2000,labelCol="label", featuresCol="features")
...
# #Saving model
model.write().overwrite().save("model_finalproject_v1")
