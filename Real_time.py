...
kafkaStream = KafkaUtils.createDirectStream(ssc, ['nyc'], {'metadata.broker.list': 'localhost:9092'})                       
# kafkaStream = KafkaUtils.createStream(ssc, ['tweets'], {'metadata.broker.list': 'localhost:9092'})
...
def reception(message):
  ...
  message = literal_eval(message.replace('NaN', 'None'))
  print(message)
  return message


# lines = kafkaStream.map(lambda x: json.loads(eval(x))).map(lambda x: x.split(","))
lines = kafkaStream.map(lambda x: reception(x))
#Loading pipeline
model = PipelineModel.load('model_finalproject_v1/')
#Considered columns
